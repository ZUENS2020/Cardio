...
6. **只有左声道 / 单声道**（实测踩过的坑）：八成是**耳机线插头是 4 段(TRRS,带麦/线控)**，
   而 PCM5102 的座是 **3 段(TRS)** —— 4 极头插到底会错位丢右声道（插一半反而左右都有，但接触不稳）。
   **换一根普通 3 段无麦音频线，或加 TRRS→TRS 转接头**即可。先在手机上插同一根线对比能快速确认是线的问题。
   （固件侧已验证 I2S 上 L/R 双声道数据正常，排除软件原因。）

> 切换/持久化：控制台 `out pcm5102`（热切，免重启）；或 `config set audio_output pcm5102` + `config save`
> （写入 SD `/Cardio/config.txt`，开机自动走 DAC）。切回内置：`out internal`。
