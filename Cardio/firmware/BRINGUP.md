...
## 3. JackMonitor — 结论：无硬件检测，降级处理

**实测结论（2026-06-01）：**
- 扫描 GPIO 0/1/2/21（M5Unified 源码推断的候选引脚）
- 插拔耳机前后全部保持高电平（1），无任何变化
- M5Unified 源码也无 ADV jack-detect GPIO 定义
- **ADV 3.5mm 插孔的机械触点仅负责模拟音频路由，不连接任何 MCU 引脚**

**降级方案（已实施）：**
- JackMonitor 保留代码框架（JACK_PIN=-1 禁用检测）
- 插入耳机时硬件自动切断扬声器（模拟机械开关），无需固件干预
- 自动暂停功能推入后续迭代（需外接检测电路或改硬件）
- UI 保留手动暂停/继续按键

**验收：**
- [x] GPIO 扫描完成，确认无可用引脚
- [x] JackMonitor 代码框架就位（可未来启用）
