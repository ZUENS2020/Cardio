...
gantt
    ...
    section Week 1 固件核心
    Logger + DebugConsole（Serial）      :done, w1a0, 2026-05-31, 1d
    Config + AudioEngine + JackMonitor   :w1a, after w1a0, 2d
    Playlist + LocalSource + PlayOrder   :w1b, after w1a, 2d
    PlayerScreen + BrowserScreen + UI    :w1c, after w1b, 2d
    BLE（BleProvisioner + BlePushService）:w1d, after w1c, 1d

    section Week 2 通知功能
    NotifyManager（BLE 路径）+ NotifyOverlay + CallScreen :w2a, 2026-06-07, 2d
    WiFiManager + RssSource              :w2b, after w2a, 2d

    section Week 2 客户端
    Android（通知监听 + BLE 直推）       :a1, 2026-06-07, 4d
...
| NVS 断电续播 | 1d |
| 省电息屏 + 低电警告 | 1d |
| 自定义开屏动画（GIF）/ 静态图（JPG 回退） | 1d |
| Splash Converter 工具（支持导出 GIF + JPG） | 1.5d |
| macOS 客户端 | 3d |
| Windows 客户端 | 3d |
| iOS 客户端（如有需要） | 5d |
| Web 配置页面 | 待定 |
...
### 固件端（SplashScreen.h/cpp，~80 行）

启动时优先检测 `/Cardio/splash.gif`，存在则播放动画（最多 5s 或 5 次循环）；
GIF 不存在则检测 `/Cardio/splash.jpg` 显示静态图 2s；两者都不存在直接进主界面。
任意键跳过，进入后立即显示 PlayerScreen。

**GIF 路径（主路径）：**
- 库：`bitbank2/AnimatedGIF`（PlatformIO 直接安装，Apache-2.0 开源）
- GIFDraw 回调写入全屏 Sprite，叠加 boot log 文字后 `pushSprite(0,0)`，实现动画背景 + 实时 boot log 叠加效果
- 解码在 Core 1 异步任务运行；Turbo 模式 buffer 分配到 PSRAM（约 60KB），240×135 @ 15-25fps
