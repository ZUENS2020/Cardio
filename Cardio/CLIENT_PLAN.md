...
graph LR
    ...
    subgraph macOS
        M1[NotificationMonitor\n读取 Notification Center\nSQLite 数据库]
        M2[Uploader\nURLSession POST]
        M1 --> M2
    end

    subgraph Windows
        W1[UserNotificationListener\nWinRT API]
        W2[Uploader\nHttpClient POST]
        W1 --> W2
    end

    A4 & M2 & W2 -->|HTTP POST JSON| Server[FastAPI 服务端]
    Server -->|MQTT publish| MQ[Mosquitto]
    MQ -->|CF Tunnel WSS| Card[Cardputer ADV]
    Card -->|BLE GATT Notify\nreq-wifi / req-hotspot| A5
    A5 -->|BLE GATT Write\nSSID + Password| Card
```
...
- [ ] `BleWifiHelper`：后台 BLE 扫描，监听 GATT Notify，响应 req-wifi / req-hotspot
- [ ] `WifiCredential`：DataStore 存取 SSID→密码映射，热点凭据缓存
- [ ] 设置页：服务器地址、白名单、**已知 WiFi 密码列表**、测试连接、总开关
